...
# CSGO Dockerfile
#
# Also see: https://hub.docker.com/r/austinsaintaubin/docker-csgoserver/
...
RUN sed -i '/emailalert=/s/"\([^"]*\)"/"$EMAIL_NOTIFICATION"/' csgoserver && \
    ...
    sed -i '/gslt=/s/"\([^"]*\)"/"$AUTH_KEY"/' csgoserver && \
